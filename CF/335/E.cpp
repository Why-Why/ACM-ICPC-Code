...
// Author        : WhyWhy
// Created Time  : 2015年12月10日 星期四 19时39分46秒
// File Name     : E.cpp
...
int main()
{
	...
	scanf("%d %lf %lf",&N,&dic.x,&dic.y);

	for(int i=0;i<N;++i) scanf("%lf %lf",&list[i].x,&list[i].y),maxX=max(maxX,list[i].x),maxY=max(maxY,list[i].y);
	list[N++]=Point(0,maxY);
	list[N++]=Point(maxX,0);
	//list[N++]=Point(0,0);
	...
	Line t;
	Line L(Point(0,0),dic);
	Point p;

	for(int i=0;i<top;++i) {
		t=Line(list[Stack[i]],list[Stack[(i+1)%top]]);
		if(Seg_inter_Line(L,t)) {
			p=(L & t).second;
			printf("%.10f\n",dic.x/p.x);
			return 0;
		}
