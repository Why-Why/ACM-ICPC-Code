...
// Author        : WhyWhy
// Created Time  : 2015年10月29日 星期四 17时50分09秒
// File Name     : M.cpp
...
int main()
{
	...
	while(~scanf("%s",s))
	{
		...
		for(int i=rank[M+1];i>1;--i)
		{
			minn=min(minn,height[i]);
			if(sa[i-1]<M && minn*2>=M-sa[i-1])			// !!!
			{
				ans=minn;
				break;
			}
		...
		minn=0x3f3f3f3f;
		for(int i=rank[M+1]+1;i<=N;++i)
		{
			minn=min(minn,height[i]);
			if(sa[i]<M && minn*2>=M-sa[i])				// !!!
			{
				ans=max(ans,minn);
				break;
			}
