...
class DAGConstruction {
    ...
    vector<int> construct(vector<int> x) {
		int N=x.size();
		vector <int> noans,ans;
		noans.push_back(-1);

		vector <pii> rem;
		for(int i=0;i<N;++i) rem.push_back(pii(x[i],i));
		sort(rem.begin(),rem.end());

		for(int i=0;i<N;++i) {
			if(rem[i].first>i+1) return noans;
			for(int j=0;j<rem[i].first-1;++j)
				ans.push_back(rem[i].second),ans.push_back(rem[j].second);
		}
