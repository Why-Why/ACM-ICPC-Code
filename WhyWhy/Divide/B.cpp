...
void solve(int ql,int qr,int L,int R) {
	if(ql>qr) return;
	if(L==R) {
		for(int i=ql;i<=qr;++i) if(ope[i].type==1) ans[ope[i].id]=L;
		return;
	}
	...
	for(int i=ql;i<=qr;++i)
		...
		else if(ope[i].type==2) { if(ope[i].b<M) add(ope[i].a,-1); }
...
int main()
{
	...
	while(T--) {
		...
		solve(1,len,-1,INF);
		for(int i=1;i<=couans;++i) printf("%d\n",ans[i]);
	}
